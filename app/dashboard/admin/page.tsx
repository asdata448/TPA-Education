...
export default async function AdminDashboardPage() {
  ...
  return <main className="container mx-auto space-y-8 px-6 py-12">
    ...
    <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>Tutors</CardTitle><CardDescription>Review Tutor accounts, status, and profile details.</CardDescription></div><Link className="text-sm font-medium text-primary underline-offset-4 hover:underline" href="/dashboard/admin/tutors">Open Tutor CRUD</Link></div></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Status</TableHead><TableHead>Subjects</TableHead><TableHead>Phone</TableHead><TableHead>Action</TableHead></TableRow></TableHeader><TableBody>{tutors.length===0?<TableRow><TableCell colSpan={5} className="py-8 text-center text-muted-foreground">No Tutors yet.</TableCell></TableRow>:tutors.map(t=><TableRow key={t.tutorId}><TableCell className="font-medium">{t.fullName}</TableCell><TableCell><Badge variant={t.profileActive&&t.tutorActive?'default':'secondary'}>{t.profileActive&&t.tutorActive?'Active':'Inactive'}</Badge></TableCell><TableCell>{t.subjects||'-'}</TableCell><TableCell>{t.phone||'-'}</TableCell><TableCell><Link className="text-sm font-medium text-primary underline-offset-4 hover:underline" href={`/dashboard/admin/tutors/${t.tutorId}`}>View / Edit</Link></TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
  </main>
}
