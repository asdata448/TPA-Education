...
function pickOne<T>(value: T | T[] | null | undefined): T | null { if (!value) return null; return Array.isArray(value) ? value[0] ?? null : value }
...
export async function getAdminFeedbackItems(): Promise<AdminFeedbackItem[]> {
  ...
  const { data, error } = await admin
    ...
    .select('id,kind,message,status,reject_reason,admin_note,handled_at,created_at,tutors(id,profiles(full_name)),teaching_material_library_items(title)')
